...
def run_nikto(onion_link, index):
        nikto_command = f"proxychains -q nikto -h {onion_link} -Tuning 2,3,5,0,a,b -Display 3,S -maxtime 10s -o nikto_log/vuln{index}.json 2>/dev/null"
        subprocess.run(nikto_command, shell=True)
        return colored(f"\nNikto scan for {onion_link} completed.\n", "yellow")

def run_feroxbuster(onion_link, index):

    feroxbuster_command = f"feroxbuster -u {onion_link} -w wordlist.txt --proxy socks5h://127.0.0.1:9050 --threads 30 -C 404 --time-limit 30s --auto-bail -q 2>/dev/null"
...
def run_backend_enum():
    ...
    for index, onion_link in enumerate(onion_links, start=1):
        ...
        thread = threading.Thread(target=run_nikto, args=(onion_link, index))
        threads.append(thread)
        thread.start()
